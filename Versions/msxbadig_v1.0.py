...
remrkLbl = args.lr
labelLin = args.ll
if remrkLbl.upper() == 'S': remrkLbl = "'"
remrkReg = args.rr
if remrkReg.upper() == 'S': remrkReg = "'"
remrkChg = args.cr
identKep = args.ki
idtspKep = args.ci
identSze = args.si
...
# array[:] = [re.sub(r'##.*".*".*$', '', c).rstrip()+'\n' for c in array] # works with quotes after ## but do not work with two sets of quotes


if verbos > 0:
	print '--- delete or REM blank lines and convert existing REMs'
